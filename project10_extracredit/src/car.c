...
int car_cmp(struct car *a, struct car *b) {
    ...
    if (makeCompare > 0) {
        ...
        int modelCompare = strcmp(a->model, b->model);

        if (modelCompare > 0) {
            return -1;
        } else if (modelCompare < 0) {
            return 1;
        } else {

            //Then compare the color of the two car structures
            int colorCompare = strcmp(a->color, b->color);
            ...
            if (colorCompare > 0) {
                ...
                //Finally, compare the year

                int aYear = a->year;
                int bYear = b->year;

                return (aYear > bYear) ? -1 : (aYear < bYear);
